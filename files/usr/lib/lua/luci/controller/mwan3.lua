...
function diagnosticsData(interface, task)
	...
	function diag_speed(cmd)
                local result = io.popen(cmd)
		if result then
                	luci.http.write("Result:\n")
			while true do
				local ln = result:read("*l")
				if not ln then break end
				if not string.find(ln, "ISP") then
					luci.http.write(ln)
                			luci.http.write("\n")
				end
			end
	...
	if device then
		...
		elseif task == "speedtest_int" then
			local cmd = io.popen(string.format("ifconfig %s 2>/dev/null|awk '/inet addr:/ {print $2}'|sed 's/addr://'", device))
			local ip = cmd:read("*a")
			cmd:close()
			diag_speed(string.format("curl http://192.168.8.1/lua/speedtest%%20--accept-license%%20--progress=no%%20-i%%20%s", ip))
		else
